...
//! is unit-testable. Lifted verbatim from switchblade's `sb-window`
//! (where a headless benchmark runner drives the same function); abner's
//! `about_to_wait` is its only caller. The rules are load-bearing — the
//! occlusion guard is a pegged-core bug — and the tests below pin them.
//!
...
/// Redraw cadence while idle — housekeeping (decoder health, hover
/// timeouts) without burning CPU on a paused frame.
